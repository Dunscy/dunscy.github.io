...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    ...
    (0, util_2.checkPetrify)(gen, attacker, defender);
    (0, util_2.checkPetrify)(gen, defender, attacker);
    ...
    (0, util_2.checkInfiltrator)(attacker, field.defenderSide, move);
    (0, util_2.checkInfiltrator)(defender, field.attackerSide, move);
    var desc = {
    ...
    switch (defender.ability) {
        case 'Ethereal Shroud':
            if ((0, util_2.getMoveEffectiveness)(gen, move, 'Ghost', isGhostRevealed, field.isGravity, isRingTarget) > 1) {
                abilitytypemod = 1;
            }
            else
                abilitytypemod = (0, util_2.getMoveEffectiveness)(gen, move, 'Ghost', isGhostRevealed, field.isGravity, isRingTarget);
            desc.defenderAbility = defender.ability;
            break;
        case 'Ivy Wall':
            if ((0, util_2.getMoveEffectiveness)(gen, move, 'Grass', isGhostRevealed, field.isGravity, isRingTarget) > 1) {
                abilitytypemod = 1;
            }
            else
    ...
    var typeEffectiveness = type1Effectiveness * type2Effectiveness * abilitytypemod;
...
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    if ((move.named('Facade') && attacker.hasStatus('brn', 'par', 'psn', 'tox')) ||
        ...
        var isGhostRevealed = attacker.hasAbility('Scrappy') || attacker.hasAbility('Mind\'s Eye') ||
            field.defenderSide.isForesight;
        var isRingTarget = defender.hasItem('Ring Target') && !defender.hasAbility('Klutz');
        var types_1 = defender.teraType ? [defender.teraType] : defender.types;
        var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, types_1[0], isGhostRevealed, field.isGravity, isRingTarget);
        var type2Effectiveness = types_1[1] ? (0, util_2.getMoveEffectiveness)(gen, move, types_1[1], isGhostRevealed, field.isGravity, isRingTarget) : 1;
...
function calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc) {
    ...
    if ((attacker.hasAbility('Slow Start') && attacker.abilityOn &&
        ...
        (attacker.hasAbility('Pure Focus') && move.category === 'Special') ||
...
function calculateDefenseSMSSSV(gen, attacker, defender, move, field, desc, isCritical) {
    ...
    if (field.hasWeather('Fallout') && defender.hasType('Nuclear')) {
        defense = (0, util_2.pokeRound)((defense * 4) / 3);
        desc.weather = field.weather;
    }
...
function calculateBaseDamageSMSSSV(gen, attacker, defender, basePower, attack, defense, move, field, desc, isCritical) {
    ...
    if (field.hasWeather('Sun') && move.named('Hydro Steam') && !attacker.hasItem('Utility Umbrella')) {
        ...
        else if ((field.hasWeather('Thunderstorm') && move.hasType('Electric'))) {
            baseDamage = (0, util_2.pokeRound)((0, util_2.OF32)(baseDamage * 6144) / 4096);
            desc.weather = field.weather;
        }
    }
